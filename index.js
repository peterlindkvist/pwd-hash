...
PwdHash.simplechars = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789";
PwdHash.extrachars = "~@#%^&*()-_=+[]{}|;:,.<>/?!";
PwdHash.usestrong = true;
PwdHash.colors = ['#91c579', '#4185f4', '#ffda5c'];

PwdHash.prototype.init = function(container){
  var domain = document.location.host.split(".").splice(-2).join(".");
  this.render(container, container ? '' : domain);
  this.addEvents();
  this.inputSalt.focus();
}

PwdHash.prototype.render = function(container, domain){
  var dc = document.create;
  var inputStyle = {
    height: '30px',
    width: '130px',
    margin: '2px',
  ...
  var containerStyle = {
    ...
    top: 0,
    left: 0,
    backgroundColor: PwdHash.colors[0],
    zIndex: 100000, 
    margin: '0px',
    padding: '2px'
  }
  
  this.container = !!container ? container : PwdHash.createElement('div', containerStyle , document.body);
  
  this.inputDomain = PwdHash.createElement('input', inputStyle , this.container);
  this.inputSalt = PwdHash.createElement('input', inputStyle , this.container);
  this.inputStrong = PwdHash.createElement('input', inputStyle , this.container);
  this.inputWeak = PwdHash.createElement('input', inputStyle , this.container);
  
  this.inputDomain.setAttribute('placeholder', 'domain');
  this.inputDomain.setAttribute('value', domain);
  this.inputSalt.setAttribute('type', 'password');
  this.inputSalt.setAttribute('placeholder', 'password');
  this.inputStrong.setAttribute('type', 'password');
  this.inputWeak.setAttribute('type', 'password');
};

PwdHash.prototype.addEvents = function(){ 
  PwdHash.addEvent(this.inputDomain, ['keyup', 'paste', 'change'],this.update.bind(this));
  PwdHash.addEvent(this.inputSalt, ['keyup', 'paste', 'change'], this.update.bind(this));
  PwdHash.addEvent(this.inputStrong, ['blur', 'focus'], this.focus.bind(this));
  PwdHash.addEvent(this.inputWeak, ['blur', 'focus'], this.focus.bind(this));
}
...
PwdHash.prototype.update = function(){
  ...
  this.inputStrong.value = stronghash;
  this.inputWeak.value = weakhash;
  var color = Math.abs(CryptoJS.SHA3(salt).words[0]) % 3;
  this.container.style.backgroundColor = PwdHash.colors[color];

  this.updatePasswords();
}

PwdHash.prototype.focus = function(evt){  
  var el = evt.currentTarget;
  this.usestrong = evt.type === 'focus' &&  el === this.inputStrong;
  el.setAttribute('type', evt.type === 'focus' ? 'text' : 'password');
  if(evt.type === 'focus'){
    this.updatePasswords();  
    el.select();
...
PwdHash.prototype.updatePasswords = function(){
  var hash = this.usestrong ? this.inputStrong.value : this.inputWeak.value;
  var elems = document.querySelectorAll('input[type=password]');
  var el, i;

  for(var i = 0; i < elems.length ; i ++){
    el = elems[i];
    if(el !== this.inputStrong && el !== this.inputWeak && el !== this.inputSalt){
      el.value = hash;
    }
  };
}

PwdHash.createElement = function(tag, styles, parent){
...
PwdHash.addEvent = function(el, events, callback){
  events.forEach(function(event){
    el.addEventListener(event, callback);
  });
};

PwdHash.validate = function(hash, checkextrachars){
...
PwdHash.generate = function(domain, pwd, omitextrachars, len){
  var chars = PwdHash.simplechars + (omitextrachars ? '' : PwdHash.extrachars);
  ...
  for(i = 0; i < 100; i++){
    ...
    if(PwdHash.validate(hash, !omitextrachars)){
      return hash;
    }
  }
  return "";
